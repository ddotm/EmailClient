...
2.3 [x] Normalize config validation behavior across providers.
...
3.2 [x] Preserve provider implementation order strictly as: SMTP, then Mailgun, then SendGrid.
...
4.1 [x] Add first-class generic SMTP provider (host/port/security/auth configurable).
4.2 [x] Add SendGrid provider using official API patterns and robust request/response/error handling.
4.3 [x] Align Mailgun provider with shared abstractions and remove provider-specific behavior leaks from the main app contract.
4.4 [x] Ensure each provider has its own project and depends on IdempotentCookie.Email (Infrastructure) only.
...
6.2 [x] Reduce SRP/DRY violations across provider implementations.
...
7.1 [x] Expand unit tests to cover SMTP, SendGrid, and Mailgun request composition, validation, cancellation, and failure paths.
...
9. Confirm provider implementations are complete in required order: SMTP, Mailgun, SendGrid.

**Decisions**
- Package name: IdempotentCookie.Email
- Namespaces: IdempotentCookie.Email.DependencyInjection, IdempotentCookie.Email.Smtp, IdempotentCookie.Email.Mailgun, IdempotentCookie.Email.SendGrid
...
- Provider implementation order is fixed: SMTP > Mailgun > SendGrid.
...
## DIMA'S NOTES
- SMTP > Mailgun > SendGrid - that is the preferred implementation order, nothing else. There will be no failover whatsoever. Take failover provisions out of the spec.
- Convert .sln to .slnx 
...
## Decision
1. Provider implementation and delivery order is fixed to SMTP, then Mailgun, then SendGrid.
2. Runtime behavior is explicit provider selection by configuration type (for example: SmtpConfig, MailgunConfig, SendGridConfig).
